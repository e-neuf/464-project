...
def get_per_point_model(scope, P, n_max_instances, is_training, bn_decay):
    ...
    n_registered_primitives = fitter_factory.get_n_registered_primitives()  #number of registered primitives
    with tf.variable_scope(scope):
        #construct a pointnet++ model with set abstraction layers and feature propagation layers, and fully connected layers
        ...
        W, normal_per_point, type_per_point = net_results #output list of tensors 
        #W: segemtnation instance; fractional tensor that indicates liklihood of a point belonging to a particular primitive
    ...
    fitter_feed = {
        'P': P, #poind cloud
        'W': W, #segmentation instance (normalized)
        'normal_per_point': normal_per_point,
    }
    
    #create additional parameters for point cloud data based on pointnet++ model
    #planeFitter: normals, offset 
    #sphereFitter: center, radius
    #cylinderFitter: center, radius, axis
    #coneFitter: center, radius, axis, angle
    parameters = {}
